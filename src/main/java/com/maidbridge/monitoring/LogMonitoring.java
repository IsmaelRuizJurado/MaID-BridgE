...
public class LogMonitoring implements LineMarkerProvider {
    ...
    public void collectSlowLineMarkers(@NotNull List<? extends PsiElement> elements,
        ...
        ZonedDateTime threshold = switch (settings.getLogTimeRange()) {
            case "7d" -> ZonedDateTime.now().minusDays(7);
            case "30d" -> ZonedDateTime.now().minusDays(30);
            case "custom" -> settings.getLogCustomTime();
        ...
        for (PsiElement element : elements) {
            ...
            for (Map.Entry<LogKey, LogData> entry : logCounts.entrySet()) {
                ...
                if (data.logTime == null || !data.logTime.isAfter(threshold)) {
                    continue;
                }
        ...
        try {
            ...
            for (int i = 0; i < hits.length(); i++) {
                ...
                counts.compute(key, (k, v) -> {
                    ...
                    if (v == null) return new LogData(1, isRecent ? 1 : 0, logTime);
                    v.count++;
